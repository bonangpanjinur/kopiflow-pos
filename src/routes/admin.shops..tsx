...
function AdminShopDetail() {
  ...
  const doReset = async () => {
    ...
    try {
      ...
      toast.success(`Link reset dikirim ke ${(res as { email: string }).email}`);
    } catch (e) {
      toast.error((e as Error).message);
    } finally {
      setBusy(false);
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-6 lg:py-10 space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            {s.name}
            {isSuspended && <Badge variant="destructive">Dinonaktifkan</Badge>}
      ...
      <Card className="p-5">
        ...
        <p className="text-xs text-muted-foreground mt-1">Override langsung tanpa membuat tagihan. Gunakan untuk kasus komplimen / koreksi.</p>
        ...
        <div className="mt-2 text-xs text-muted-foreground">Pendapatan bulan ini dari toko: {formatIDR(0)} (lihat /admin/invoices)</div>
      </Card>
