...
function AdminDomains() {
  ...
  const reload = async () => {
    const { data } = await supabase.from("businesses").select("id, name, slug, custom_domain, custom_domain_verified_at").not("custom_domain", "is", null).order("created_at", { ascending: false });
    setRows((data as Row[]) ?? []);
  };
  useEffect(() => { reload(); }, []);
