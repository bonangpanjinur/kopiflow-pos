...
FOR ALL TO authenticated
USING (EXISTS (
  SELECT 1 FROM stock_opnames o JOIN coffee_shops s ON s.id = o.shop_id
...
FOR ALL TO authenticated
USING (EXISTS (SELECT 1 FROM coffee_shops s WHERE s.id = menu_reviews.shop_id AND s.owner_id = auth.uid()))
WITH CHECK (EXISTS (SELECT 1 FROM coffee_shops s WHERE s.id = menu_reviews.shop_id AND s.owner_id = auth.uid()));
