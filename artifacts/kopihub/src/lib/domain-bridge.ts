...
export async function requestCustomDomainBridge({ data }: { data: { domain: string } }) {
  ...
  const token = `flowpos-verify=${Math.random().toString(36).slice(2)}`;
  const { error } = await supabase.from("businesses").update({ custom_domain: domain, custom_domain_verify_token: token, custom_domain_verified_at: null }).eq("id", shop.id);
  if (error) throw error;
  return { token, instructions: `Add TXT record _flowpos-verify.${domain} = ${token}` };
}
...
const CNAME_TARGET = "tenants.flowpos.app";

export async function verifyCustomDomainBridge(): Promise<VerifyResult> {
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) throw new Error("not_authenticated");
  const { data: shop } = await supabase.from("businesses").select("id, custom_domain, custom_domain_verify_token").eq("owner_id", user.id).maybeSingle();
  ...
  try {
    const [txtRes, cnameRes] = await Promise.all([
      fetch(`https://1.1.1.1/dns-query?name=_flowpos-verify.${domain}&type=TXT`, { headers: { Accept: "application/dns-json" } })
    ...
    if (txtFound) {
      await supabase.from("businesses").update({ custom_domain_verified_at: new Date().toISOString() }).eq("id", shop.id);
      base.verified = true;
    }
...
export async function removeCustomDomainBridge() {
  ...
  const { data: shop } = await supabase.from("businesses").select("id").eq("owner_id", user.id).maybeSingle();
  if (!shop) throw new Error("shop_not_found");
  await supabase.from("businesses").update({ custom_domain: null, custom_domain_verify_token: null, custom_domain_verified_at: null }).eq("id", shop.id);
  return { removed: true };
}
